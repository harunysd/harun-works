...
export function parseArticleBody(value) {
  ...
  while (i < lines.length) {
    ...
    const h1 = line.match(HEADING_1_PATTERN);
    ...
    // Headings (check h3 first, then h2, then h1)
    if (h3) {
      flushParagraph();
    ...
    if (unordered) {
      ...
      i++;
      while (i < lines.length) {
        if (!lines[i].trim() && i + 1 < lines.length && lines[i + 1].trim().match(UNORDERED_LIST_PATTERN)) {
          i++;
          continue;
        }
    ...
    if (ordered) {
      ...
      i++;
      while (i < lines.length) {
        if (!lines[i].trim() && i + 1 < lines.length && lines[i + 1].trim().match(ORDERED_LIST_PATTERN)) {
          i++;
          continue;
        }
